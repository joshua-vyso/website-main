...
import { RecommendationsView } from '@/components/platform/pricepilot/RecommendationsView';
import {
  pickBaseList,
  productMargins,
...
export default async function PricePilotRecommendationsPage() {
  ...
  // Units sold per product over the last 30 days (drives opportunity impact).
  ...
  const opportunities = computeOpportunities(pms, target, units30d);

  return (
    <RecommendationsView opportunities={opportunities} baseListId={base?.id ?? null} target={target} />
  );
}
